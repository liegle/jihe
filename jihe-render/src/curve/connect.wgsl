@group(0)
@binding(0)
var intersection_texture: texture_storage_2d_array<rgba8unorm, read>;
...
fn cs(@builtin(global_invocation_id) id: vec3<u32>) {
    let here = textureLoad(intersection_texture, id.xy, id.z);
    let l = here.yw;
    let t = here.xz;
    let r = textureLoad(intersection_texture, id.xy + vec2<u32>(1, 0), id.z).yw;
    let b = textureLoad(intersection_texture, id.xy + vec2<u32>(0, 1), id.z).xz;
    ...
    textureStore(segment_texture, id, pq);
}
