...
fn cs(@builtin(global_invocation_id) id: vec3<u32>) {
    let dst_size = textureDimensions(intersection_texture) - vec2<u32>(1, 1);
    let lrtb = vec4<f32>(
        (f32(id.x) - f32(dst_size.x) / 2 - .5) * camera.scale + camera.pos.x,
        (f32(id.x) - f32(dst_size.x) / 2 + .5) * camera.scale + camera.pos.x,
        (f32(dst_size.y) / 2 - f32(id.y) + .5) * camera.scale + camera.pos.y,
        (f32(dst_size.y) / 2 - f32(id.y) - .5) * camera.scale + camera.pos.y,
    );
...
fn binary(pa: vec2<f32>, pb: vec2<f32>) -> vec2<f32> {
    ...
    if fa == 0 || fb == 0 || fa == fb { return vec2<f32>(0, 1); }
